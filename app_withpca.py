...
def analysis(issue, state, county):
	...
	N=len(data_train)
	GLR_R2=GLR.score(X, y_train)


	LRR = Ridge(alpha=0.1)
	LRR.fit(X, y_train)
	LRR_R2=LRR.score(X, y_train)

	LLR = Lasso(alpha=0.0005)
	LLR.fit(X, y_train)
	LLR_R2=LLR.score(X, y_train)
...
#
	print('GLR:',GLR_R2, GLR.coef_)
	print('Ridge: ', LRR_R2, LRR.coef_)
	print('Lasso: ', LLR_R2, LLR.coef_)
	#print('KNN: ', KNN_R2, scoresKNN)
	#print('DTR: ', scoresRFR, RFR.feature_importances_)
	...
	for place in locations:
		...
		PredictXPCA=pca.transform(PredictX)
		GLR_predict=GLR.predict(PredictXPCA)*100.0
